...
module.exports = {
	Direct: function (t, a) {
		...
		a(router.call(event, '/'), false);

		a.deep(router.call(event, '/milo/foo/'),
			{ conf: { controller: conf['/'] }, result: undefined, event: event });
		a.deep(called, ['root']);
		clear.call(called);

		a.deep(router.call(event, '/milo/foo/foo/'),
			{ conf: { controller: conf.foo }, result: undefined, event: event });
		a.deep(called, ['foo']);
		clear.call(called);

		a(router.call(event, '/milo/foo/miszka'), false);
		a.deep(called, []);

		a.deep(router.call(event, '/milo/foo/marko'),
			{ conf: { controller: conf.marko }, result: undefined, event: event });
		a.deep(called, ['marko']);
		clear.call(called);

		a.deep(router.call(event, '/milo/foo/bar/dwa'),
			{ conf: { controller: conf['bar/dwa'] }, result: obj, event: event });
		a.deep(called, ['bar/dwa']);
		clear.call(called);

		a.deep(router.call(event, '/milo/foo/elo/dwa'), { conf: { controller: conf['elo/dwa'] },
			result: undefined, event: event });
		a.deep(called, ['elo/dwa']);
		clear.call(called);

		a(router.call(event, '/milo/foo/elo/dwa/marko'), false);
		a.deep(called, ['elo/dwa/*:match']);
		clear.call(called);

		a.deep(router.call(event, '/milo/foo/elo/dwa/foo'),
			{ conf: conf['elo/dwa/[a-z]+'], result: undefined, event: event });
		a.deep(called, ['elo/dwa/*:match', 'elo/dwa/*:controller']);
		clear.call(called);

		a(router.call(event, '/milo/foo/elo/dwa/foo/foo/ilo'), false);
		a.deep(called, ['elo/dwa/*/foo/*:match2']);
		clear.call(called);

		a.deep(router.call(event, '/milo/foo/elo/dwa/foo/foo/bar'),
			{ conf: conf['elo/dwa/[a-z]+/foo/[a-z]+'], result: undefined, event: event });
		a.deep(called, ['elo/dwa/*/foo/*:match2', 'elo/dwa/*/foo/*:controller']);
		clear.call(called);

		a(router.call(event, '/milo/foo/elo/dwa/abla/bar'), false);
		a.deep(called, []);

		a(router.call(event, '/milo/foo/elo/dwa/foo/bar/miko'), false);
		a.deep(called, []);

		a.deep(router.call(event, '/milo/foo/elo/dwa/filo'),
			{ conf: { controller: conf['elo/dwa/filo'] }, result: undefined, event: event });
		a.deep(called, ['elo/dwa/filo']);
		clear.call(called);

		a.deep(router.call(event, '/milo/foo/elo/trzy'), { conf: { controller: conf['elo/trzy'] },
			result: undefined, event: event });
	...
	Match: function (t, a) {
		...
		a.deep(router.call(event, '/milo/binio/foo/'), false);

		a.deep(router.call(event, '/milo/elos/foo/'), { conf: { controller: conf['/'],
			match: nestConf['milo/[a-z]+/foo'].match }, result: undefined, event: event });
		a.deep(called, ['root']);
		clear.call(called);

		a.deep(router.call(event, '/milo/elos/foo/foo/'), { conf: { controller: conf.foo,
			match: nestConf['milo/[a-z]+/foo/foo'].match }, result: undefined, event: event });
		a.deep(called, ['foo']);
		clear.call(called);

		a(router.call(event, '/milo/elos/foo/miszka'), false);
		a.deep(called, []);

		a.deep(router.call(event, '/milo/elos/foo/marko'), { conf: { controller: conf.marko,
			match: nestConf['milo/[a-z]+/foo/marko'].match }, result: undefined, event: event });
		a.deep(called, ['marko']);
		clear.call(called);

		a.deep(router.call(event, '/milo/elos/foo/bar/dwa'), { conf: { controller: conf['bar/dwa'],
			match: nestConf['milo/[a-z]+/foo/bar/dwa'].match },
			result: obj, event: event });
		a.deep(called, ['bar/dwa']);
		clear.call(called);

		a.deep(router.call(event, '/milo/elos/foo/elo/dwa'), { conf: { controller: conf['elo/dwa'],
			match: nestConf['milo/[a-z]+/foo/elo/dwa'].match },
			result: undefined, event: event });
		a.deep(called, ['elo/dwa']);
		clear.call(called);

		a(router.call(event, '/milo/elos/foo/elo/dwa/marko'), false);
		a.deep(called, ['elo/dwa/*:match']);
		clear.call(called);

		a.deep(router.call(event, '/milo/elos/foo/elo/dwa/foo'),
		...
		a(router.call(event, '/milo/elos/foo/elo/dwa/foo/foo/ilo'), false);
		a.deep(called, ['elo/dwa/*/foo/*:match2']);
		clear.call(called);

		a.deep(router.call(event, '/milo/elos/foo/elo/dwa/foo/foo/bar'),
		...
		a(router.call(event, '/milo/elos/foo/elo/dwa/abla/bar'), false);
		a.deep(called, []);

		a(router.call(event, '/milo/elos/foo/elo/dwa/foo/bar/miko'), false);
		a.deep(called, []);

		a.deep(router.call(event, '/milo/elos/foo/elo/dwa/filo'),
		...
		a.deep(router.call(event, '/milo/elos/foo/elo/trzy'), { conf: { controller: conf['elo/trzy'],
